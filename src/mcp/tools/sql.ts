import { runSqlAudit, type RunSqlOutcome } from "../../cli/commands/sql.js";
import type { OmgResponse } from "./types.js";

export const sqlAuditTool = {
  name: "omg.sql.audit",
  description: "Read-only Cloud SQL instance and backup audit for a Google Cloud project.",
  inputSchema: {
    type: "object",
    properties: {
      project: { type: "string" },
...
export async function handleSqlAudit(args: unknown): Promise<OmgResponse> {
  const parsed = parseAuditArgs(args);
  if (!parsed.ok) {
    return parsed.response;
  }

  return fromOutcome("sql:audit", await runSqlAudit(parsed.args));
}
...
function fromOutcome(command: string, outcome: RunSqlOutcome): OmgResponse {
  if (outcome.ok) {
    return {
      ok: true,
      command,
