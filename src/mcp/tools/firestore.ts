import { runFirestoreAudit, type RunFirestoreOutcome } from "../../cli/commands/firestore.js";
import type { OmgResponse } from "./types.js";

export const firestoreAuditTool = {
  name: "omg.firestore.audit",
  description: "Read-only Firestore database and composite index audit for a Google Cloud project.",
  inputSchema: {
    type: "object",
    properties: {
      project: { type: "string" },
...
export async function handleFirestoreAudit(args: unknown): Promise<OmgResponse> {
  const parsed = parseAuditArgs(args);
  if (!parsed.ok) {
    return parsed.response;
  }

  return fromOutcome("firestore:audit", await runFirestoreAudit(parsed.args));
}
...
function fromOutcome(command: string, outcome: RunFirestoreOutcome): OmgResponse {
  if (outcome.ok) {
    return {
      ok: true,
      command,
