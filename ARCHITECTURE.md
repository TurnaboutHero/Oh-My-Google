...
- `src/connectors/budget-api.ts` contains injected Budget API request execution, post-verification core, and token/HTTP failure mapping, but it is not wired to `budget ensure --yes`.
