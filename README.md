...
`oh-my-google`은 AI 코딩 에이전트가 Google Cloud, Firebase, 그리고 이후 더 넓은 Google 서비스들을 하나의 안전한 작업 단위로 다루게 하는 CLI + MCP 하네스입니다.
...
장기 방향은 Google Cloud와 Firebase의 무료 프로그램과 무료 등급을 가능한 한 잘 활용하게 돕고, 그 다음 Google Workspace, Maps, Analytics, YouTube, Ads, Gemini/Vertex 같은 다양한 Google 서비스도 같은 안전 하네스 아래로 붙이는 것입니다. 다만 무료 체험/무료 등급은 제품별 정책과 실제 사용량에 따라 달라지므로, `omg`는 "무료 보장" 대신 공식 문서 기반의 보수적 분류, dry-run, budget guard, local cost lock, 서비스별 `unknown` 판정을 기본값으로 둡니다.
...
- Cloud Run, Firebase Hosting, Firestore, Cloud Storage for Firebase, Secret Manager, Billing Budget은 서로 다른 API와 권한을 사용합니다.
- Firebase Hosting, Firestore, Storage 같은 부차 서비스도 각각 다른 quota, rules, IAM, 과금 표면을 가집니다.
...
- Budget Pub/Sub notification audit와 dry-run routing planning
...
- free-tier-aware GCP+Firebase service coverage 방향을 PRD/PLAN/runbook에 반영

실제 검증된 범위:

- disposable GCP 프로젝트에서 `init -> link -> deploy -> doctor` E2E 검증 완료
- 해당 disposable 프로젝트는 검증 후 삭제 요청 상태까지 확인됨
...
- local cost lock이 활성화된 프로젝트는 위 cost-bearing live 작업이 budget audit 전에 `COST_LOCKED`로 차단됩니다.
...
- `omg`는 MCP server이고, 등록/allowlist된 read-only tool만 통과시키는 좁은 downstream MCP gateway도 갖고 있습니다.
- budget 생성/수정 live 실행은 production CLI 런타임에서 아직 열지 않았습니다. 현재는 audit, Budget API enable, `budget ensure --dry-run` 정책 계획, injected Budget API executor core, live gate contract, approval/decision-log command-core wiring, transport failure mapping, opt-in transport factory, mock-only live tests, budget notification audit/dry-run, Pub/Sub topic/IAM read-only audit, budget alert to cost lock ingestion dry-run plan을 지원합니다. Pub/Sub topic/IAM setup, alert ingestion setup, live agent IAM bootstrap은 manual-first 경계로 둡니다.
- Firestore, Cloud Storage, Cloud SQL, live IAM write/provisioning과 외부 `notify` sender surface는 아직 설계/구현 전입니다.
- Free-tier guidance command/output은 아직 설계 단계입니다. Firebase Hosting, Firestore, Cloud Storage for Firebase, Cloud Run, Cloud Build, Artifact Registry, logging, egress를 별도 서비스 표면으로 분류하는 방향만 문서화되어 있습니다.
- Google Cloud/Firebase 무료 정책은 제공자 문서에 따라 바뀔 수 있습니다. `omg` 문서는 가격 숫자를 고정하지 않고 공식 문서 링크와 `unknown` 판정을 기본 안전값으로 사용합니다.
...
- downstream MCP write/lifecycle proxy는 구체적인 verifier가 생기기 전까지 구현하지 않습니다.
- Firebase Hosting, Firestore, Cloud Storage for Firebase를 "Firebase"라는 단일 덩어리가 아니라 별도 audit/plan/free-tier-risk 표면으로 다룹니다.
- 이후 Google Workspace, Maps Platform, Analytics, YouTube, Ads 같은 broader Google services도 OAuth scope, 사용자 데이터 접근, quota, 비용, 승인 경계를 별도 서비스 표면으로 분류해 붙입니다.
- 무료 등급 관련 판단은 [Google Cloud free program documentation](https://docs.cloud.google.com/free/docs/free-cloud-features?hl=ko)을 출발점으로 하되, 정책이 모호하거나 최신성이 확인되지 않으면 `unknown`으로 남깁니다.
...
| `omg.budget.ensure` | budget policy dry-run plan; live mutation blocked |
| `omg.budget.notifications.audit` | budget Pub/Sub notification routing audit |
| `omg.budget.notifications.ensure` | budget Pub/Sub notification routing dry-run plan; live mutation blocked |
| `omg.budget.notifications.lock_ingestion` | budget alert to local cost lock ingestion dry-run plan |
| `omg.cost.status` | local cost lock status 조회 |
| `omg.cost.lock` | local cost lock 설정 |
| `omg.cost.unlock` | explicit-confirm local cost lock 해제 |
| `omg.firestore.audit` | Firestore database/index audit |
| `omg.iam.audit` | IAM policy/service account audit |
| `omg.iam.plan` | separated agent IAM identity plan |
| `omg.iam.bootstrap` | separated agent IAM bootstrap dry-run; live mutation blocked |
| `omg.security.audit` | read-only project/IAM/budget security posture audit |
| `omg.sql.audit` | Cloud SQL instance/backup audit |
| `omg.storage.audit` | Cloud Storage bucket/IAM audit |
| `omg.mcp.gateway.audit` | registered downstream MCP tool audit/discovery |
| `omg.mcp.gateway.call` | allowlisted read-only downstream MCP tool call |
...
- [docs/runbooks/phase-4-4b-release-notes.md](./docs/runbooks/phase-4-4b-release-notes.md): Phase 4/4B release notes
...
- active cost lock은 cost-bearing live 작업의 hard blocker로 취급합니다.
