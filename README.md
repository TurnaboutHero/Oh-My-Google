...
- live `omg deploy`, `omg firebase deploy --execute`, Secret Manager write 전 budget guard 적용
...
- budget guard는 현재 live `omg deploy`, `omg firebase deploy --execute`, `omg secret set`에 강제 적용됩니다.
- `init`과 API enable 같은 first-run/setup 경로에는 아직 비용 가드 설계가 남아 있습니다.
- budget 생성/수정은 아직 구현하지 않았습니다. 현재는 audit과 Budget API enable만 지원합니다.
- `iam`, `notify`, `security` admin surface는 아직 설계/구현 전입니다.
