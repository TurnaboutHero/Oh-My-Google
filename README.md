...
- budget 생성/수정 live 실행은 아직 구현하지 않았습니다. 현재는 audit, Budget API enable, `budget ensure --dry-run` 정책 계획, injected Budget API executor core, live gate contract, transport failure mapping, opt-in transport factory, budget notification audit/dry-run, Pub/Sub topic/IAM read-only audit, budget alert to cost lock ingestion dry-run plan을 지원합니다. Pub/Sub topic/IAM setup, alert ingestion setup, live agent IAM bootstrap은 manual-first 경계로 둡니다.
