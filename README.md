...
- budget 생성/수정 live 실행은 production CLI 런타임에서 아직 열지 않았습니다. 현재는 audit, Budget API enable, `budget ensure --dry-run` 정책 계획, injected Budget API executor core, live gate contract, transport failure mapping, opt-in transport factory, mock-only command-core wiring, budget notification audit/dry-run, Pub/Sub topic/IAM read-only audit, budget alert to cost lock ingestion dry-run plan을 지원합니다. Pub/Sub topic/IAM setup, alert ingestion setup, live agent IAM bootstrap은 manual-first 경계로 둡니다.
