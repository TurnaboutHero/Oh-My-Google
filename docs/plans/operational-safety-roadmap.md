...
- `budget ensure --yes` remains blocked in the production CLI runtime with `BUDGET_ENSURE_LIVE_NOT_IMPLEMENTED`; the injected Budget API executor core, live gate contract, hash-bound approval consumption, decision logging, transport failure mapping, opt-in transport factory, and mock-only command-core wiring can exist before live CLI wiring is opened.
