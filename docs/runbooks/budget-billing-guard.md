...
Live budget creation/update is still blocked in the current safe implementation. `budget ensure --yes` returns `BUDGET_ENSURE_LIVE_NOT_IMPLEMENTED`; the injected Budget API executor core exists, but live transport/auth wiring and post-verification failure handling remain deferred.
